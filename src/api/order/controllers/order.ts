  ...
  ({ strapi }) => ({
      ...
      for (const item of items) {
        if (!item.productDocumentId) {
          return ctx.badRequest(
            "productDocumentId is required for each order item",
          );
        }
        ...
        if (qty <= 0) {
          return ctx.badRequest(
            `Quantity must be positive for ${item.productName ?? "product"}`,
          );
        }
    ...
    async regenerateSnapToken(ctx) {
      ...
      try {
        ...
        strapi.log.info(
          `Snap token regenerated for order ${order.orderNumber}`,
        );
    ...
    async delete(ctx) {
      ...
      const entity = await strapi.service("api::order.order").delete(id);
